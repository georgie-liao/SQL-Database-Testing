...
//		System.out.println(shipped+" "+canceled+" "+resolved+" "+disputed); //Print the output of above stored procedures
		...
		int exp_shipped=cStmt.getInt("shipped");
		int exp_canceled=cStmt.getInt("canceled");
	...
	void test_getCustomerShipping() throws SQLException
	{
		...
		Statement stmt=con.createStatement();
		rs=stmt.executeQuery("SELECT country, CASE WHEN country = 'USA' THEN '2-day Shipping' WHEN country = 'Canada' THEN '3-day Shipping' ELSE '5-day Shipping' END as ShippingTime FROM customers WHERE customerNumber = 121");
		
		rs.next();
		
		String exp_shippingTime=cStmt.getString("ShippingTime");
